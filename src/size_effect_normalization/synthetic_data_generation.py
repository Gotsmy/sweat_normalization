import numpy as np
from . import extended_model as em
...
def sample_sweat_volumes(nr):
    ...
    ----------
    nr       : Int
               Number of sweat volumes to sample.

    Returns
    -------
    sampels  : List
               List of sampled sweat volumes
    '''
...
def generate_sweat_volumes(n_replicates,n_metabolites,n_timepoints):
    ...
    ----------
    n_replicates  :  Int
                     Number of synthetic data replicates.
    n_metabolites :  Int.
    ...
    Returns
    -------
    sv_tensor     :  numpy.ndarray
                     Array of shape (n_metabolites, n_timepoints) of sampled sweat volumes.
                     Sweat volumes are duplicated along the n_metabolites axis.
    sv_vector     :  numpy.ndarray
                     Array of shape (n_timepoints) of sampled sweat volumes.
    '''
...
def generate_experimental_errors(n_replicates,n_metabolites,n_timepoints,error_sigma):
    ...
    n_timepoints   : Int
                     Number of measured time points.
    error_sigma    : Float
                     Float of relative error SD of the synthetic data.

    Returns
    -------
    e_tensor      :  numpy.ndarray
                     Array of shape (n_metabolites, n_timepoints).
    '''
...
def generate_random_kinetic_data(n_known_metabolites,n_metabolites,toy_parameters,timepoints,bounds_per_metabolite):
    ...
    Parameters
    ----------
    n_known_metabolites   : Int
                            Number of metabolites in the toy model, in this study 4.
    n_metabolites         : Int
                            Total number of metabolites for which measured data needs to be generated.
    toy_parameters        : numpy.ndarray
                            Array of shape (5, n_known_parameters) with parameters of basic toy model.
    timepoints            : numpy.ndarray
                            Array of shape (n_timepoints) of time points.
    bounds_per_metabolite : numpy.ndarray
                            Array of shape (5) of upper bounds of kinetic constants for one metabolite.
                            Kinetic parameters are sampled between 0 and bounds_per_metabolite for n_metabolites
                            - n_known_metabolites.

    Returns
    -------
    c_tensor              : numpy.ndarray
                            Array of shape (n_metabolites, n_timepoints)
    '''
...
def generate_random_from_real_data(n_known_metabolites,n_metabolites,toy_parameters,timepoints,bounds_per_metabolite):
    ...
    location = pkg_resources.resource_stream(__name__,'data/normalized_20_timepoints.csv')
    norm_tmp = pd.read_csv(location,index_col=0)

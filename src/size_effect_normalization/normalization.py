...
def calculate_pqn(m_tensor):
    ...
    Parameters
    ----------
    m_tensor: numpy.ndarray
              Array of shape (n_metabolites * n_timepoints) of measured data.

    Returns
    -------
    pqn    : numpy.ndarray
             Array of shape (n_timepoints) with normalization factors.
    '''
...
def calculate_pkm(m_tensor,lb,ub,timepoints,n_metabolites,n_cpu,n_replicates,loss_name,trans_fun,lambda_):
    ...
    m_tensor      :   numpy.ndarray
                      Array of shape (n_metabolites * n_timepoints) of measured data.
    lb            :   numpy.ndarray
                      Array of shape (n_metabolites * 5 + n_timepoints) of lower bounds of parameters for model fitting.
    ub            :   numpy.ndarray
                      Array of shape (n_metabolites * 5 + n_timepoints) of upper bounds of parameters for model fitting.
    timepoints    :   numpy.ndarray
                      Array of shape (n_timepoints) of time points.
    n_metabolites :   Int
                      Number of metabolites to fit.
    n_cpu         :   Int
                      Number of CPUs used in multiprocessing.
    n_replicates  :   Int
                      Number of Monte Carlo replicates used for optimization.
    loss_name     :   Str
                      Loss used for optimization.
    trans_fun     :   Str
                      Trans_fun argument of PKM_model
    lambda_       :   Float
                      Weighting value for loss calculation.
...
def calculate_mix(m_tensor,sv_pqn,lb,ub,timepoints,n_metabolites,n_cpu,n_replicates,loss_name,trans_fun,scale_fun,lambda_):
    ...
    m_tensor      :   numpy.ndarray
                      Array of shape (n_metabolites * n_timepoints) of measured data.
    sv_pqn        :   numpy.ndarray
                      Array of shape (n_timepoints) of normalization factors as calculated from PQN.
    lb            :   numpy.ndarray
                      Array of shape (n_metabolites * 5 + n_timepoints) of lower bounds of parameters for model fitting.
    ub            :   numpy.ndarray
                      Array of shape (n_metabolites * 5 + n_timepoints) of upper bounds of parameters for model fitting.
    ...
    loss_name     :   Str
                      Loss used for optimization.
    trans_fun     :   Str
                      Trans_fun argument of MIX_model
    scale_fun     :   Str
                      Scale_fun argument of MIX_model
    lambda_       :   Float
                      Weighting value for loss calculation.

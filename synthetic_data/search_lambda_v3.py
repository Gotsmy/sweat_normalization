...
if __name__ == "__main__":
    ...
    ## END INPUT PARAMETERS ##
            ...
            results_time  = {'original':[],'0.01':[],'0.33':[],'0.66':[],'0.99':[]}
            results_sv    = {'original':[],'0.01':[],'0.33':[],'0.66':[],'0.99':[],'TRUE':[]}
            results_model = {'original':[],'0.01':[],'0.33':[],'0.66':[],'0.99':[]}
            raw_values    = {'C':[],'SV':[],'M':[]}
            for n_replicate in range(n_replicates):
                ...
                l1_sv, l1_model = norm.calculate_mix(m_tensor[:4,:],sv_pqn,
                ...
                loss_name = 'cauchy_loss'
                sv_pqn                      = norm.calculate_pqn(m_tensor)
                l2_sv, l2_model = norm.calculate_mix(m_tensor[:4,:],sv_pqn,
                                                            mini_lb,mini_ub,timepoints,n_known_metabolites,
                                                            n_cpu,n_mc_replicates,loss_name,'log10',
                                                            'standard',.01)
                t3 = time.time()
                loss_name = 'cauchy_loss'
                sv_pqn                      = norm.calculate_pqn(m_tensor)
                l3_sv, l3_model = norm.calculate_mix(m_tensor[:4,:],sv_pqn,
                                                            mini_lb,mini_ub,timepoints,n_known_metabolites,
                                                            n_cpu,n_mc_replicates,loss_name,'log10',
                                                            'standard',.33)
                t4 = time.time()
                loss_name = 'cauchy_loss'
                sv_pqn                      = norm.calculate_pqn(m_tensor)
                l4_sv, l4_model = norm.calculate_mix(m_tensor[:4,:],sv_pqn,
                                                            mini_lb,mini_ub,timepoints,n_known_metabolites,
                                                            n_cpu,n_mc_replicates,loss_name,'log10',
                                                            'standard',.66)
                t5 = time.time()
                loss_name = 'cauchy_loss'
                sv_pqn                      = norm.calculate_pqn(m_tensor)
                l5_sv, l5_model = norm.calculate_mix(m_tensor[:4,:],sv_pqn,
                                                            mini_lb,mini_ub,timepoints,n_known_metabolites,
                                                            n_cpu,n_mc_replicates,loss_name,'log10',
                                                            'standard',.99)
                t6 = time.time()
                
                results_time['original'].append(t2-t1)
                results_time['0.01'].append(t3-t2)
                results_time['0.33'].append(t4-t3)
                results_time['0.66'].append(t5-t4)
                results_time['0.99'].append(t6-t5)
                
                results_sv['original'].append(l1_sv)
                results_sv['0.01'].append(l2_sv)
                results_sv['0.33'].append(l3_sv)
                results_sv['0.66'].append(l4_sv)
                results_sv['0.99'].append(l5_sv)
                results_sv['TRUE'].append(sv_vector)
                
                results_model['original'].append(l1_model)
                results_model['0.01'].append(l2_model)
                results_model['0.33'].append(l3_model)
                results_model['0.66'].append(l4_model)
                results_model['0.99'].append(l5_model)
            ...
            with open(f'lambda_results/v3_e_{error_sigma}_n_{n_metabolites}.pkl','wb') as file:
